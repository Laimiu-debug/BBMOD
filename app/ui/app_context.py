...
def _default_fox_zip() -> Path | None:
    for coll in _collection_candidates():
        for p in coll.rglob("data狐狸汉化*.zip"):
            return p
    return None
...
class AppContext(QObject):
    ...
    @property
    def fox_reference_zip(self) -> Path | None:
        p = self.settings.get("fox_reference_zip")
        if p and Path(p).exists():
            return Path(p)
        found = _default_fox_zip()
        if found:
            return found
        return None
