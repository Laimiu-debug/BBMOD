this.afei_m08_escort_contract <- this.inherit("scripts/contracts/contract", {
	m = {
		Destination = null,
		Title = "再集合·护送滞留者"
	},
	function create()
	{
		...
		this.createStates();
		this.createScreens();
		this.m.Type = "contract.afei_m08_escort";
		this.m.Name = "再集合·护送滞留者";
	...
	function createStates()
	{
		...
		this.m.States.push({
			...
			function start()
			{
				...
				if (camp != null)
				{
					try { camp.setName("滞留者追兵"); } catch (errorName) { camp.m.Name = "滞留者追兵"; }
					camp.onSpawned();
	...
	function createScreens()
	{
		this.m.Screens.push({
			ID = "Task",
			Title = this.m.Title,
			Text = "%terrainImage%{还有人滞留在旧营地附近，需要护送离开。}",
			Image = "",
			List = [],
			Options = [
				{
			...
			Options = [
				{
					...
					function getResult()
					{
						this.World.Flags.set("afei_active_contract_combat", "contract.afei_m08_escort");
