this.afei_m02_review_event <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Screens.push({
			ID = "A",
			Text = "%terrainImage%{一次合作失误后，阿飞想继续哇哇叫，抹茶让他先听每个人说完。}",
			Image = "", List = [], Characters = [],
			Options = [
				{ Text = "一起复盘（6 食物、磨合 +6）", function getResult(_event) {
		...
		this.m.Screens.push({ ID="Done", Text="%terrainImage%{话说完了。此后普通营地复盘可满足成长条件。}", Image="", List=[], Characters=[],
