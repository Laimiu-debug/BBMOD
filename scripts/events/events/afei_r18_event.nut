this.afei_r18_event <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Screens.push({
			ID = "A",
			Text = "%terrainImage{旧剧场那边托来护送委托。美伢在侧幕整理袖口，说完成相遇后可付 [color=#8f2525]300[/color] 克朗入册；线索可留。\n\n暂缓不消失；满员或缺钱时可晚些再见。}",
		...
		this.m.Screens.push({
			ID = "Drill",
			Text = "%terrainImage{她把让位的步伐走给你们看：命中之后，要给队友留格子。}",
			Image = "", List = [], Characters = [],
			Options = [
				{ Text = "谈签约。", function getResult(_event) { return "B"; } },
				{ Text = "今天先停。", function getResult(_event) { this.World.Flags.set(::AfeiExpedition.Flags.R18Offered, 1); return 0; } }
			],
			function start(_event) {}
		});
		this.m.Screens.push({
			ID = "B",
			Text = "%terrainImage{签约费 [color=#8f2525]300[/color] 克朗。身份 C21，入队 1 级 0 经验。}",
			Image = "", List = [], Characters = [],
			Options = [
		...
		this.m.Screens.push({
			...
			Options = [{ Text = "欢迎入队。", function getResult(_event) {
						...
						if (!::AfeiExpedition.tryChargeHire("C21", 300))
						{
							return "Poor";
						}
