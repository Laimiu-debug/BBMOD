this.afei_r20_event <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Screens.push({
			ID = "A",
			Text = "%terrainImage{英灵殿 003 号来信。千涵收好短坡的鞋带，说来过后可付 [color=#8f2525]280[/color] 克朗签约。\n\n暂缓不消失；满员或缺钱时可晚些再见。}",
		...
		this.m.Screens.push({
			ID = "Drill",
			Text = "%terrainImage{她说冲刺可以短，方向不能糊。}",
			Image = "", List = [], Characters = [],
			Options = [
				{ Text = "谈签约。", function getResult(_event) { return "B"; } },
				{ Text = "今天先停。", function getResult(_event) { this.World.Flags.set(::AfeiExpedition.Flags.R20Offered, 1); return 0; } }
			],
			function start(_event) {}
		});
		this.m.Screens.push({
			ID = "B",
			Text = "%terrainImage{签约费 [color=#8f2525]280[/color] 克朗。身份 C23，入队 1 级 0 经验。}",
			Image = "", List = [], Characters = [],
			Options = [
