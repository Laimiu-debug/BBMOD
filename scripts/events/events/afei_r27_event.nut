this.afei_r27_event <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Screens.push({
			ID = "A",
			Text = "%terrainImage{城镇球场被占道的委托贴在墙上。罗一可卡在门前空档，说来过后可付 [color=#8f2525]350[/color] 克朗。\n\n暂缓不消失；满员或缺钱时可晚些再见。}",
		...
		this.m.Screens.push({
			ID = "Drill",
			Text = "%terrainImage{方向可以交给别人。她先把角度选干净。}",
			Image = "", List = [], Characters = [],
			Options = [
				{ Text = "谈签约。", function getResult(_event) { return "B"; } },
				{ Text = "今天先停。", function getResult(_event) { this.World.Flags.set(::AfeiExpedition.Flags.R27Offered, 1); return 0; } }
			],
			function start(_event) {}
		});
		this.m.Screens.push({
			ID = "B",
			Text = "%terrainImage{签约费 [color=#8f2525]350[/color] 克朗。身份 C30，入队 1 级 0 经验。}",
			Image = "", List = [], Characters = [],
			Options = [
		...
		this.m.Screens.push({
			...
			Options = [{ Text = "欢迎入队。", function getResult(_event) {
						...
						if (!::AfeiExpedition.tryChargeHire("C30", 350))
						{
							return "Poor";
						}
