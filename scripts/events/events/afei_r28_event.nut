this.afei_r28_event <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Screens.push({
			ID = "A",
			Text = "%terrainImage{商旅公用钱袋要核账。bula 把预算摊开，说来过后可付 [color=#8f2525]360[/color] 克朗；暂缓保留。\n\n暂缓不消失；满员或缺钱时可晚些再见。}",
		...
		this.m.Screens.push({
			ID = "Drill",
			Text = "%terrainImage{预算有人一起看，才不会变成一个人的秘密。}",
			Image = "", List = [], Characters = [],
			Options = [
				{ Text = "谈签约。", function getResult(_event) { return "B"; } },
				{ Text = "今天先停。", function getResult(_event) { this.World.Flags.set(::AfeiExpedition.Flags.R28Offered, 1); return 0; } }
			],
			function start(_event) {}
		});
		this.m.Screens.push({
			ID = "B",
			Text = "%terrainImage{签约费 [color=#8f2525]360[/color] 克朗。身份 C31，入队 1 级 0 经验。}",
			Image = "", List = [], Characters = [],
			Options = [
