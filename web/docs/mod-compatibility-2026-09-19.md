...
`其他mod/` 曾被纳入公开仓库，含 81 个第三方归档或相关文件。本次先从当前 Git 索引移除并加入忽略规则，再经用户明确授权、完整备份、隔离过滤和逐提交核查，对 main 执行受保护强推。所有公开分支与标签的可达历史中已没有该目录，本地 83 份收藏文件保留。旧发布标签不含该目录，哈希未改变。

GitHub 仍保留可通过旧哈希访问的对象，不能把此次历史重写称为平台已彻底删除所有旧副本。证据、恢复与其他克隆的同步方式见 [历史清理记录](git-history-cleanup-2026-09-19.md)。
